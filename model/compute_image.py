...
def generate_patch(boxes,threshold):
    ...
    """
        boxes: boxes tensor(num_boxes,4)
...
def random_patching(img,boxes,labels):
    """
        chọn ngẫu nhiên patch cho ảnh, sau đó lấy những path có iou lớn hơn ngưỡng và có điểm trung tâm thuộc gt_box # thuong thi 200 box có iou lớn nhất
            img: original PIL image
            boxes: gt boxes,
            labels: gt labels boxes
        return -> 
            boxes: path_box (new_box,4)
            label: label path box (new_box,4)
    ...
    box_centers = (boxes[:,:2]+boxes[:,2:])/2 #[x_center,y_center]
    keep_idx = (
        (ious > 0.3) & 
        (box_centers[:,0] > patch[0]) &
        (box_centers[:,1] > patch[1]) &
        (box_centers[:,2] < patch[2]) &
        (box_centers[:,3] < patch[3]) 
    )
